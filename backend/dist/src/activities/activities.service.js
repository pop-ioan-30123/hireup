...
let ActivitiesService = class ActivitiesService {
  ...
  `;
    ...
    validatePayload(payload) {
        ...
        if (!categoryKey) {
            throw new common_1.BadRequestException('Missing categoryKey');
        }
    ...
    async markNotificationsRead(userId, auditContext) {
        return this.db.withTransaction(async (client) => {
            await (0, audit_sql_context_1.applyAuditSqlContext)(client, {
                currentUserId: userId,
                ...auditContext,
    ...
    async update(userId, activityId, payload, auditContext) {
        ...
        `, [activityId]);
            ...
            const validated = this.validatePayload({
                ...
                categoryKey: (payload.categoryKey ?? current['categoryKey']),
                subcategoryKey: (payload.subcategoryKey ?? current['subcategoryKey']),
