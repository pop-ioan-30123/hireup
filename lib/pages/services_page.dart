...
class _ServicesPageState extends State<ServicesPage> {
  ...
  Future<void> _refreshActivitiesFromApi({bool silent = false}) async {
    ...
    try {
      ...
      final notifications = await ApiService.listActivityNotifications(
        accessToken: token,
      );
  ...
  List<_ActivityItem> _postedItems() {
    return _items.where((item) => item.isPostedByCurrentUser).toList();
  }

  List<_ActivityItem> _upcomingItems() {
    return _items.where((item) => _appliedByMe.contains(item.id)).toList();
  }
      ...
      child: AspectRatio(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final outline = _romaniaOutlinePath(mapSize);
            final regions = _buildCountyRegions(mapSize, outline);
            ...
            return MouseRegion(
              onExit: (_) {
                if (_hoveredCounty != null) {
                  setState(() => _hoveredCounty = null);
                }
              },
              onHover: (event) {
                final hovered = hitTest(event.localPosition);
                if (hovered != _hoveredCounty) {
                  setState(() => _hoveredCounty = hovered);
                }
              },
              child: GestureDetector(
                behavior: HitTestBehavior.opaque,
                onTapUp: (details) {
                  final selectedCounty = hitTest(details.localPosition);
                  if (selectedCounty == null) return;
                  _onCountySelectedFromMap(selectedCounty);
                },
                child: CustomPaint(
                  painter: _RomaniaCountyMapPainter(
                    colorScheme: scheme,
                    outlinePath: outline,
                    regions: regions,
  ...
  Path _romaniaOutlinePath(Size size) {
    final points = _romaniaOutlinePoints
  ...
  List<_CountyRegion> _buildCountyRegions(Size size, Path outlinePath) {
    final regionScale = size.shortestSide * 0.055;

    final regions = <_CountyRegion>[];
    for (final county in RomaniaLocations.counties) {
      final anchor = _countyAnchors[county] ?? const Offset(0.5, 0.5);
      final center = Offset(anchor.dx * size.width, anchor.dy * size.height);

      final seed = county.runes.fold<int>(0, (sum, rune) => sum + rune);
      final vertices = <Offset>[];

      for (var i = 0; i < 6; i++) {
        final angle = (i * 60.0 - 20.0) * (3.14159265 / 180.0);
        final wobble = 0.82 + (((seed + i * 13) % 10) / 25.0);
        final radius = regionScale * wobble;
        vertices.add(
          Offset(
      ...
      final rawPath = Path()..moveTo(vertices.first.dx, vertices.first.dy);
      for (var i = 1; i < vertices.length; i++) {
        rawPath.lineTo(vertices[i].dx, vertices[i].dy);
      }
      rawPath.close();

      final clipped = Path.combine(PathOperation.intersect, rawPath, outlinePath);
      if (clipped.computeMetrics().isEmpty) continue;

      regions.add(_CountyRegion(county: county, path: clipped));
    }
...
class _CountyRegion {
  ...
  const _CountyRegion({required this.county, required this.path});
}

class _RomaniaCountyMapPainter extends CustomPainter {
  final ColorScheme colorScheme;
  final Path outlinePath;
  final List<_CountyRegion> regions;
  final String? selectedCounty;
  final String? hoveredCounty;

  const _RomaniaCountyMapPainter({
    required this.colorScheme,
    required this.outlinePath,
    required this.regions,
    required this.selectedCounty,
    required this.hoveredCounty,
  });

  Path _scaledPath(Path source, double scale) {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final countyBorderPaint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 0.9
      ..color = colorScheme.outline.withValues(alpha: 0.45);

    for (final region in regions) {
      final isSelected = region.county == selectedCounty;
      final isHovered = region.county == hoveredCounty;
      final scale = isHovered ? 1.1 : 1.0;
      final regionPath = _scaledPath(region.path, scale);
      ...
      final fillPaint = Paint()
        ...
        ..color = isSelected
            ...
            : isHovered
            ? colorScheme.secondary.withValues(alpha: 0.52)
            : colorScheme.surfaceContainerHighest.withValues(alpha: 0.48);

      canvas.drawPath(regionPath, fillPaint);
      canvas.drawPath(regionPath, countyBorderPaint);

      if (isHovered || isSelected) {
        final centroid = regionPath.getBounds().center;
  ...
  @override
  bool shouldRepaint(covariant _RomaniaCountyMapPainter oldDelegate) {
    return oldDelegate.selectedCounty != selectedCounty ||
        oldDelegate.hoveredCounty != hoveredCounty ||
        oldDelegate.regions != regions ||
        oldDelegate.colorScheme != colorScheme;
  }
}

const List<Offset> _romaniaOutlinePoints = [
  Offset(0.05, 0.45),
  Offset(0.09, 0.30),
  Offset(0.16, 0.18),
  Offset(0.28, 0.08),
